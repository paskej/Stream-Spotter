...
namespace UnitTests
{
    ...
    public class RecommendationsTests
    {
        ...
        static string[] SERVICES = { "netflix", "disney", "hulu", "prime" };
        [TestMethod]
        public void GetRecommendations()
        {

            Result[] results = JsonConvert.DeserializeObject<RootObject>(WITCHER).results;
            Result[] rec = recommendations.getRecommendations(results, SERVICES);
            Assert.IsTrue(rec.Length > 0);
        }

        [TestMethod]
        public void GetRecommendationsWISHLISTNULL()
        {
            Result[] results = null;
            Result[] rec = recommendations.getRecommendations(results, SERVICES);
            Assert.AreEqual(rec, null,"Failed handling a null wishlist");
        }
        [TestMethod]
        public void GetRecommendationsSERVICESNULL()
        {
            Result[] results = JsonConvert.DeserializeObject<RootObject>(WITCHER).results;
            Result[] rec = recommendations.getRecommendations(results, null);
            Assert.AreEqual(rec, null, "Failed handling a null wishlist");
        }
